...
def _load_credentials() -> dict:
    """
    Carica le credenziali GitHub da secrets.env.
    
    Cerca il file in:
      1. Directory corrente (./secrets.env)
      2. Directory del progetto (../secrets.env)
      3. Home directory (~/secrets.env)
    """
    # Cerca il file secrets.env in più posti
    candidates = [
    ...
    for candidate in candidates:
        ...
        raise FileNotFoundError(
            "File secrets.env non trovato. Cercato in:\n" +
    ...
    if missing:
        raise ValueError(
            f"Credenziali mancanti in secrets.env: {missing}"
        )
...
def _generate_viewer_id(prefix: str = "A3F") -> str:
    """
    Genera un ID univoco per il viewer.
    
    Formato: A3F-XXXXXX
    Dove:
      - A3F = prefisso ArtiFix
      - XXXXXX = 6 caratteri alfanumerici casuali
    
    Esempio: A3F-XK9M2P
    """
    alphabet = string.ascii_uppercase + string.digits
    # Rimuovi caratteri ambigui (0/O, 1/I/L)
...
def _upload_to_github(
    ...
    """
    Fa upload del file HTML su GitHub.
    
    Returns:
        URL pubblico del viewer.
    """
    try:
        from github import Github, GithubException
    except ImportError:
        raise ImportError(
            "PyGithub non installato. Esegui: pip install PyGithub"
        )
    ...
    # Verifica se il file esiste già (in tal caso lo aggiorna)
    try:
...
def share_html_viewer(
    ...
    *,
    ...
    Returns:
        dict con:
          - viewer_id: ID univoco generato
          - url: URL pubblico del viewer
          - qr_path: percorso del QR Code (se generate_qr=True)
          - uploaded_at: timestamp dell'upload
    """
    # 1. Leggi HTML
...
def _cli() -> None:
    ...
    print("=" * 60)
    print()
    print("Apri il link nel browser per vedere il 3D.")
